...
public class FriendDbStorage implements FriendStorage {
    ...
    private List<User> processFriendsQuery(String sql, Object[] params) {
        return jdbcTemplate.query(sql, params, (rs, rowNum) -> User.builder().id(rs.getInt("user_id")).email(rs.getString("user_email")).login(rs.getString("user_login")).name(rs.getString("user_name")).birthday(rs.getDate("user_birthday").toLocalDate()).friendsId(getFriendsIds(rs.getInt("user_id"))).build());
    }
